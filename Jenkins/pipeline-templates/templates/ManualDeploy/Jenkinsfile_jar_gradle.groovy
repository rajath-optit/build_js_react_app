...
tools {
    ...
    parameters {
        ...
        string(name: 'REMOTE_PATH', defaultValue: '/root', description: 'The path where artifact to be copy.')
    }
    ...
    stages {
      ...
      stage('Copy Artifact to Remote') {
            steps {
               script {
                  gradleDeployScript.copyJarToRemote(params.REMOTE_HOST_CREDENTIAL, params.REMOTE_HOST, params.ARTIFACT_PATH, params.REMOTE_PATH)
            }
          }
       }
       ...
       stage('Deploy Artifact in Remote') {
            steps {
               script {
                  gradleDeployScript.deployJar(params.REMOTE_HOST_CREDENTIAL, params.REMOTE_HOST, params.REMOTE_PATH)
            }
          }
       }
    }
